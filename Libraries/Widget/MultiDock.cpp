...
public:
  ...
  HintRegion(Composite* parent) : Composite(parent)
  {
...
bool MultiDocker::StartManipulation(Widget* widget, DockMode::Enum direction)
{
  ...
  {
    new DockMovingManipulation(PL::gMouse, DockOwner, widget);
    return true;
  }
  ...
  if (Area == DockArea::Floating)
  {
    new SizingManipulation(PL::gMouse, widget, direction);
    return true;
  }

  new DockedSizingManipuation(PL::gMouse, DockOwner, widget, direction);
  return true;
}

LightningDefineType(MultiDock, builder, type)
{
}

MultiDock::MultiDock(Composite* parent) : Composite(parent)
{
...
{
  // If there is no widgets set the dock size to plasma
  // and return
  ...
  // Dock area is plasma when no widgets have been added
