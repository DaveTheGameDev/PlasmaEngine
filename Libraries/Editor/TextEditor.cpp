...
#include "ScintillaPlatformPlasma.hpp"
...
LightningDefineType(TextEditorEvent, builder, type)
{
}
...
//          So, 'ScintillaPlasma' is a container.
...
class ScintillaPlasma : public Scintilla::ScintillaBase
{
public:
...
private:
  ...
  ScintillaPlasma& operator=(const ScintillaPlasma&);
};
...
static int KeyTranslate(int keyIn)
{
  switch (keyIn)
  {
  case Plasma::Keys::Down:
    return SCK_DOWN;
  case Plasma::Keys::Up:
    return SCK_UP;
  case Plasma::Keys::Left:
    return SCK_LEFT;
  case Plasma::Keys::Right:
    return SCK_RIGHT;
  case Plasma::Keys::Home:
    return SCK_HOME;
  case Plasma::Keys::End:
    return SCK_END;
  case Plasma::Keys::PageUp:
    return SCK_PRIOR;
  case Plasma::Keys::PageDown:
    return SCK_NEXT;
  case Plasma::Keys::Delete:
    return SCK_DELETE;
  case Plasma::Keys::Escape:
    return SCK_ESCAPE;
  case Plasma::Keys::Back:
    return SCK_BACK;
  case Plasma::Keys::Tab:
    return SCK_TAB;
  case Plasma::Keys::Enter:
    return SCK_RETURN;
  case Plasma::Keys::Alt:
    return SCK_MENU;
  default:
    return keyIn;
  }
...
LightningDefineType(TextEditor, builder, type)
{
}
...
void TextEditor::SetLexer(uint lexer)
{
  ...
  case Lexer::Lightning:
  {
    const char lightningKeywords[] = "abstract alias alignof as assert Assign auto base break case catch "
                                 "checked "
    ...
    const char lightningSpecial[] = "this value event";

    SendEditor(SCI_SETLEXER, (uptr_t)SCLEX_CPP, 0);
    SendEditor(SCI_SETKEYWORDS, 0, (sptr_t)lightningKeywords);
    SendEditor(SCI_SETKEYWORDS, 1, (sptr_t)lightningSpecial);
    SendEditor(SCI_SETVIEWWS, SCWS_INVISIBLE);
    mLineNumbers = true;
    mBreakpoints = true;
    break;
...
TextEditorConfig* TextEditor::GetConfig()
{
  ...
  if (PL::gEditor)
  {
    auto config = PL::gEditor->mConfig->has(TextEditorConfig);
...
void TextEditor::OnPaste(ClipboardEvent* event)
{
  ...
  Plasma::String clipboardText = event->GetText();
  ...
  Plasma::StringRange clipboardRange = clipboardText;
...
{
  PL::gMouse->SetCursor(Cursor::Arrow);
}
                                  ...
                                  float indicatorOffsetX)
{
  ...
  for (int i = 0; i < count; ++i)
  {
    ...
    rect.Max = Vec2(indicatorOffsetX + size.x,
                    top + size.y - 1.0f); // - 1 to make it plasma-indexed
...
// Implement Scintilla in Plasma Ui
ScintillaPlasma::ScintillaPlasma()
{
  mMouseCapture = false;
...
void ScintillaPlasma::MoveSelectedLinesUp()
{
  Scintilla::UndoGroup ug(pdoc);
...
void ScintillaPlasma::MoveSelectedLinesDown()
{
  Scintilla::UndoGroup ug(pdoc);
...
int ScintillaPlasma::KeyCommand(unsigned int iMessage)
{
  switch (iMessage)
  {
  case SCI_CANCEL:
...
void ScintillaPlasma::MoveSelection(Scintilla::SelectionRange& selection, int pos, bool extend)
{
...
bool ScintillaPlasma::IsSelected(Scintilla::SelectionRange& range, int* endSelectionOut)
{
  for (size_t i = 0; i < sel.Count(); ++i)
  {
...
bool ScintillaPlasma::FindTextNotSelected(int start, int end, const char* text, Scintilla::SelectionRange& newSel)
{
...
void ScintillaPlasma::Initialise()
{
}

void ScintillaPlasma::Finalise()
{
  ScintillaBase::Finalise();
}

void ScintillaPlasma::SetVerticalScrollPos()
{
  int textHeight = SendEditor(SCI_TEXTHEIGHT);
...
void ScintillaPlasma::SetHorizontalScrollPos()
{
  mOwner->mClientOffset.x = -float(xOffset);
  mOwner->MarkAsNeedsUpdate();
}

bool ScintillaPlasma::ModifyScrollBars(int nMax, int nPage)
{
  return true;
}

void ScintillaPlasma::Copy()
{
  // This logic is now handled in OnCopy to be more browser like,
  // since we cannot explicitly access the clipboard.
}

void ScintillaPlasma::Paste()
{
}

bool ScintillaPlasma::CanPaste()
{
  return true;
}

void ScintillaPlasma::ClaimSelection()
{
}

void ScintillaPlasma::ClearHighlightRanges()
{
  ...
  //   actually
  //           walk the entire document's text.  Rather, all non-plasma style
...
void ScintillaPlasma::UpdateHighlightIndicators()
{
...
void ScintillaPlasma::ProcessTextMatch(char*& text, int* begin, int* end)
{
...
void ScintillaPlasma::HighlightMatchingText(int begin, int end, const char* text)
{
  const uint indicator = ScintillaCustomIndicators::TextMatchHighlight;
...
void ScintillaPlasma::HighlightAllTextInstances(int begin, int end, const char* text)
{
...
void ScintillaPlasma::NotifyChange()
{
  mOwner->mScinWidget->MarkAsNeedsUpdate();
}

void ScintillaPlasma::NotifyFocus(bool focus)
{
}

void ScintillaPlasma::SetTicking(bool on)
{
  if (timer.ticking != on)
    timer.ticking = on;
...
uint ScintillaPlasma::SendEditor(unsigned int Msg, unsigned long wParam, long lParam)
{
  return WndProc(Msg, wParam, lParam);
}

void ScintillaPlasma::NotifyParent(Scintilla::SCNotification scn)
{
  mOwner->OnNotify(scn);
}

void ScintillaPlasma::SetMouseCapture(bool captured)
{
...
bool ScintillaPlasma::HaveMouseCapture()
{
  return mMouseCapture;
}

void ScintillaPlasma::CreateCallTipWindow(Scintilla::PRectangle rc)
{
}

sptr_t ScintillaPlasma::DefWndProc(unsigned int iMessage, uptr_t wParam, sptr_t lParam)
{
  switch (iMessage)
  {
  case SCI_CLEAR:
...
void ScintillaPlasma::InsertAutoCompleteText(const char* text, int length, int removeCount, int charOffset)
{
  Scintilla::UndoGroup ug(pdoc);
...
void ScintillaPlasma::AddToPopUp(const char* label, int cmd /*= 0*/, bool enabled /*= true*/)
{
}

void ScintillaPlasma::UpdateSystemCaret()
{
  ScintillaBase::UpdateSystemCaret();
}

void ScintillaPlasma::CopyToClipboard(const Scintilla::SelectionText& selectedText)
{
}

void ScintillaPlasma::NotifyDoubleClick(Scintilla::Point pt, bool shift, bool ctrl, bool alt)
{
}
